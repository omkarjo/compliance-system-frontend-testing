...
export function LoadingState({ text = "Loading...", className = "" }) {
  return (
    <div
      ...
      data-state="loading"
    >
      <Hourglass className="h-12 w-12 animate-hourglass-rotate-delay text-muted-foreground" />
      <span className="mt-4 text-base font-medium text-muted-foreground">{text}</span>
    </div>
  );
}
...
export function EmptyState({ text = "No data found.", type = "empty" }) {
  ...
  const iconColor = false ? "text-destructive" : "text-muted-foreground";
  const textColor = false ? "text-destructive" : "text-foreground";
